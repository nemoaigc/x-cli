...
uv run scripts/read.py --query "<term>" --since <today> --until <tomorrow> --top 30
```
...
```bash
uv run --project ~/.claude/skills/x-cli ~/.claude/skills/x-cli/scripts/read.py \
...
Other `read.py` modes (no `--query` needed):
- `--user HANDLE` 用户最新推文
...
Before every `read.py` call, answer three questions:
