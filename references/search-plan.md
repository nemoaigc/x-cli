...
|---|---|---|---|
| **A. Follow-circle** | `read.py --user <handle>` × N | Trusted voices, authentic takes, in-community reactions | People you don't follow; follow list itself is biased |
| **B. Keyword** | `read.py --query "<term>"` | What the broader network is discussing | Wrong keyword = missed signal; viral noise |
| **C. Trending** | `digest.py --scan` → `--drill` | Algorithm-surfaced topics + KOLs | Personalized; coarse granularity |
| **D. Long-form** | `read.py --user-articles <handle>` | Articles with highest signal density | Few people write articles; low frequency |
...
uv run ~/.claude/skills/x-cli/scripts/profile.py list
# Pull follow list with bios; add --profile NAME only for multi-account setups
uv run ~/.claude/skills/x-cli/scripts/read.py --following <my_screen_name> --top 200 > /tmp/follows.json
```
...
uv run scripts/read.py --query "<core term>" --since <today> --top 30
