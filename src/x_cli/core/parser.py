...
def _entry_content(entry):
    ...
    if isinstance(content, dict):
        return content
    nested_content = _deep_get(entry, "item", "content")
    if isinstance(nested_content, dict):
        return nested_content
    return {}


def _entry_item_content(entry, content):
    # type: (Dict[str, Any], Dict[str, Any]) -> Dict[str, Any]
    item_content = content.get("itemContent")
    if isinstance(item_content, dict):
        return item_content
    nested_item_content = _deep_get(entry, "item", "itemContent")
    if isinstance(nested_item_content, dict):
        return nested_item_content
    return {}
...
def _render_article_text_block(block, entity_map):
    ...
    for offset, length, url in sorted(ranges, reverse=True):
        ...
        rendered = "%s[%s](%s)%s" % (
            rendered[:offset], safe_label, safe_url, rendered[offset + length:],
        )
    return rendered
...
def _parse_article(tweet_data):
    # type: (Dict[str, Any]) -> Dict[str, Any]
    article_results = _get_article_result(tweet_data)
    if not article_results:
        return {"article_title": None, "article_text": None}
    title = article_results.get("title")
...
def parse_tweet_result(result, depth=0):
    ...
    tweet_id = actual_data.get("rest_id", "")
    screen_name = author.screen_name
    permalink = "https://x.com/%s/status/%s" % (screen_name, tweet_id) if tweet_id and screen_name else ""
    return Tweet(
        id=tweet_id,
        ...
        is_article=bool(_get_article_result(actual_data)),
...
def parse_timeline_response(data, get_instructions):
    ...
    for instruction in instructions:
        ...
        for entry in entries:
            ...
            item_content = _entry_item_content(entry, content)
